...
public class ogmc implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) { //Check if sender is console or player
			...
			if(!(player.getGameMode() == GameMode.CREATIVE)) { //See if player is already the gamemode he/she is trying to set himself to
			player.sendMessage(Utils.chat("&6You're gamemode has been set to Creative from " + player.getGameMode())); //Sends a succesful message (Before event actually heppens to get the players gamemode)
			player.setGameMode(GameMode.CREATIVE); //Actually sets the players gamemode
		return true;
